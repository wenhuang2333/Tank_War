...
正式地图数据位于 `resource/map-now-data/1.map` ~ `6.map`（旧 `resource/map-data/` 为 20px 网格样例，不再使用）。`.map` 文件格式：

```
# 井号开头为注释行（每张地图文件头部注明名称、特色、坐标系、出生点）
...
**坐标系统：** 地图区域 1080×680 居中于窗口，左上角偏移 (150, 70)，网格 40×40（27 列 × 17 行）。.map 中的坐标为墙壁块左上角像素位置，内部可用网格 25 列 × 15 行：`x = 190 + 列×40`（190~1150），`y = 110 + 行×40`（110~670）。

**边界墙自动生成：** 地图区最外圈一层 Iron 墙壁由代码自动放置（.map 文件不写），防止坦克出界：上边 y=70、下边 y=710 两整行，左边 x=150、右边 x=1190 两整列。

**出生点：** P1 = (191, 391)（左侧中路），P2 = (1151, 391)（右侧中路），坦克 38×38 居中于格内。六张地图均保证出生点为空地，且不破坏任何墙也存在 P1→P2 通路。

**六张地图设计特色：**

| 地图 | 名称 | 特色 |
|------|------|------|
| 1 | 开阔平原 | 简单开阔，掩体稀疏，新手直接交火 |
| 2 | 中门对狙 | 中路整行通透，出生即可对狙；上下副路绕后 |
| 3 | 迷宫回廊 | 复杂迷宫，通道狭窄拐弯极多，近身遭遇战 |
| 4 | 长枪直道 | 横向长直道，拐弯极少（180° 旋转对称） |
| 5 | 十字要冲 | 十字大道贯通，四角房间蹲守，中心必争 |
| 6 | 回字堡垒 | 双层回字，内铁堡仅上下砖门可击破进入 |

除地图 4 为旋转对称外均为左右镜像对称，保证双方公平。
