...
| 资源文件 | `resource/image/`（图片资源 PNG），`resource/map-now-data/`（正式对战地图 `.map`，6 张，40px 网格）；`resource/map-data/` 为早期 20px 网格样例，不再使用 |
...
**`.map` 文件格式规范**（正式地图位于 `resource/map-now-data/1.map` ~ `6.map`）：
...
每行以 `BRICK=` 或 `IRON=` 开头，后跟分号分隔的坐标对 `x,y`，坐标为墙壁块左上角像素位置。

**坐标系统（与 pic.md 5.1 对齐）**：

| 项目 | 值 |
|------|-----|
| 地图区域 | 1080×680，居中于 1380×820 窗口，左上角偏移 (150, 70) |
| 网格 | 40×40 px，共 27 列 × 17 行 |
| 边界铁墙 | 最外圈一层（x=150 与 x=1190 两列、y=70 与 y=710 两行），代码自动生成，.map 文件不写 |
| 内部可用网格 | 25 列 × 15 行：`x = 190 + 列×40`（列 0~24），`y = 110 + 行×40`（行 0~14） |
| 出生点 | P1 = (191, 391) 左侧中路，P2 = (1151, 391) 右侧中路（坦克 38×38 居中于 40×40 格）；所有地图保证两出生点所在中路格为空地 |

**六张正式地图特色**（均保证不破坏任何墙也存在 P1→P2 通路）：

| 地图 | 名称 | 特色 | 砖墙/铁墙数 |
|------|------|------|------------|
| 1 | 开阔平原 | 简单开阔，掩体稀疏，适合新手直接交火 | 26 / 4 |
| 2 | 中门对狙 | 中路整行通透，出生即可对狙；上下副路可绕后 | 68 / 4 |
| 3 | 迷宫回廊 | 纷繁复杂，通道狭窄拐弯极多，近身遭遇战 | 144 / 0 |
| 4 | 长枪直道 | 横向长直道为主，拐弯极少，考验直线走位与换弹节奏（180° 旋转对称） | 55 / 14 |
| 5 | 十字要冲 | 十字大道贯通全图，四角房间可蹲守，中心必争 | 92 / 8 |
| 6 | 回字堡垒 | 外砖环+内铁堡双层回字，铁堡仅上下两处砖门可击破进入，攻守博弈 | 52 / 28 |

除地图 4 为 180° 旋转对称外，其余均为左右镜像对称，保证双方公平。
...
| 资源加载 | 6 张地图和所有图片能否正确加载 | `ElementLoad.init()` 无异常，地图渲染正确 |
